			...
			for (int i=0; i<f.length; i++)
			{
				...
				if (field.isAnnotationPresent(DBColumn.class))
				{
					...
					else if (field.getType() == Instant.class)
					{
						...
						if (t != null)
						{
							field.set(data, t.toInstant());
						}
					}
					else if (field.getType() == Dollar.class)
					{
						field.set(data, new Dollar(rs.getBigDecimal(columnName)));
					}
				}
				else if (field.isAnnotationPresent(DBHasOne.class))
				{
					...
					field.set(data, DB.selectOneWhere(clazz, new StringBuilder("WHERE ").append(table).append("_id=?").toString(), data.getClass().getField("id").getLong(data)));
				}
				else if (field.isAnnotationPresent(DBHasMany.class))
				{
					...
					field.set(data, DB.selectAllWhere(clazz, new StringBuilder("WHERE ").append(Util.camelToUnderscore(table)).append("_id=?").toString(), data.getClass().getField("id").getLong(data)));
				}
			}
			return data;
		}
